...
## Superloop Audit Context (.temp folder)

The `.temp/` folder contains the complete security audit for the Superloop Protocol.

### 📋 Final Audit Report

**[.temp/AUDIT_REPORT.md](.temp/AUDIT_REPORT.md)** - Complete audit report with 15 findings (2 Critical, 5 High, 6 Medium, 2 Low)

This context should be consulted before making security assessments or code changes.

### Available Context Files

| File | Purpose | Use When |
|------|---------|----------|
| `01-ARCHITECTURE_OVERVIEW.md` | System design and component relationships | Understanding overall structure |
| `02-MODULE_MAPPING.md` | All contracts, modules, and their locations | Finding specific code |
| `03-ENTRYPOINTS.md` | Public/external functions and access control | Analyzing attack surface |
| `04-ACTORS.md` | System roles and trust assumptions | Understanding permissions |
| `05-STORAGE.md` | State variables and storage layout | Analyzing state changes |
| `06-DATA_FLOWS.md` | Value and data flow diagrams | Tracing operations |
| `07-INVARIANTS.md` | System invariants and assumptions | Verifying correctness |
| `08-RISK_AREAS.md` | Identified risk areas and priorities | Focusing audit efforts |

### Key System Facts (Quick Reference)

**Core Architecture:**
- ERC4626-compliant yield vault with modular architecture
- Uses ERC-7201 namespaced storage pattern
- Module execution via `operate()` with CALL/DELEGATECALL

**Critical Actors:**
- `VaultAdmin` - Full control, can change all settings
- `VaultOperator` - Day-to-day operations via `operate()`
- `DepositManager` / `WithdrawManager` - Request queue handling
- `Accountant` - `totalAssets()` calculation (determines share price)

**High-Risk Areas:**
1. Module execution (`operate()`, `operateSelf()`)
2. Callback/fallback handling
3. Accountant price calculations
4. Share minting/burning access
5. Privilege reference counting

### Phase Status

- [x] **Phase 1**: Initial Orientation - COMPLETE
- [x] **Phase 2**: Ultra-Granular Function Analysis - COMPLETE (see `.temp/phase2/`)
- [x] **Phase 3**: Global System Understanding - COMPLETE (see `.temp/phase3/`)
- [x] **Variant Analysis**: Pattern-based vulnerability hunting - COMPLETE (see `.temp/variant-analysis/`)

### Final Audit Report

**See [.temp/AUDIT_REPORT.md](.temp/AUDIT_REPORT.md) for the complete audit report.**

**Scope Notes:**
- AccountantAaveV3 excluded (deprecated)
- Underlying protocol risks (Aave oracle) excluded
- Trust/validation issues classified as Informational
- Design choices (variable-debt-only, registered-asset-only) classified as Informational
...
| **Critical** | 0 | - |
| **High** | 0 | - |
| **Medium** | 1 | Fallback handler context |
| **Low** | 2 | Missing events, Inconsistent errors |
| **Informational** | 5 | Trust assumptions, design choices, validation |
| **Total** | 8 | - |

### Design Choices Documented

- **Variable-debt-only accounting** - Protocol only uses variable-rate borrowing
- **Registered-asset-only tracking** - Explicit asset management by operators
- **Operator trust model** - Operators assumed trusted

### Audit Context Summary

**19 documents created across 3 phases + variant analysis**

### Usage Guidelines

When working on this codebase:

1. **Before security analysis**: Read `08-RISK_AREAS.md` for prioritized focus areas
2. **Before modifying code**: Check `07-INVARIANTS.md` to avoid breaking invariants
3. **When tracing bugs**: Use `06-DATA_FLOWS.md` for operation sequences
4. **When adding features**: Consult `04-ACTORS.md` for access control patterns
