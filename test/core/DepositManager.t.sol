...
contract DepositManagerTest is TestBase {
    ...
    function test_requestDeposit_SupplyCapExceeded() public {
        ...
        vm.expectRevert(Errors.SUPPLY_CAP_EXCEEDED);
        depositManager.requestDeposit(depositAmount, address(0));
        vm.stopPrank();
    }
    ...
    function test_requestDeposit_ActiveRequestExists() public {
        ...
        vm.expectRevert(Errors.DEPOSIT_REQUEST_ACTIVE);
        depositManager.requestDeposit(depositAmount, address(0));
        vm.stopPrank();
    }
    ...
    function test_cancelDepositRequest_WrongOwner() public {
        ...
        vm.startPrank(user2);
        vm.expectRevert(Errors.CALLER_NOT_DEPOSIT_REQUEST_OWNER);
        depositManager.cancelDepositRequest(1);
        vm.stopPrank();
    }
    ...
    function test_cancelDepositRequest_AlreadyCancelled() public {
        ...
        vm.expectRevert(Errors.DEPOSIT_REQUEST_ALREADY_CANCELLED);
        depositManager.cancelDepositRequest(1);
        vm.stopPrank();
    }
    ...
    function test_cancelDepositRequest_NonExistentRequest() public {
        vm.startPrank(user1);
        vm.expectRevert(Errors.DEPOSIT_REQUEST_ALREADY_PROCESSED);
        depositManager.cancelDepositRequest(999); // Non-existent request ID
        vm.stopPrank();
    }

    // Note: Tests for already processed and partial cancellation scenarios
    // would require internal state manipulation that's not easily testable
    // without additional helper functions. These scenarios are better tested
    // through integration tests with the resolveDepositRequests function.

    // ============ Helper Functions for Testing ============

    function test_Events_Emission() public {
        uint256 depositAmount = 1000 * 10**18;
        ...
        // Test DepositRequested event
        vm.expectEmit(true, true, true, true);
        emit DepositManager.DepositRequested(user1, depositAmount, 1);
        depositManager.requestDeposit(depositAmount, address(0));
        
        // Test DepositRequestCancelled event
        vm.expectEmit(true, true, true, true);
        emit DepositManager.DepositRequestCancelled(1, user1, depositAmount);
        depositManager.cancelDepositRequest(1);
