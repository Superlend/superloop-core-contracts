...
contract MigrationTest is IntegrationBase {
    ...
    address oldVaultBtc = 0xC557529dd252e5a02E6C653b0B88984aFa3c8199;

    // currently there are 119 holder who need to be migrated to the new vault
    address[] public ALL_HOLDERS_XTZ = [
        ...
        0xf458B8FB16D661bC1E260991EBa9b1FFBC8bA39b,
    ...
    function setUp() public override {
        ...
        DataTypes.AaveV3EmodeParams memory paramsBtc = DataTypes.AaveV3EmodeParams({emodeCategory: 2});
        DataTypes.ModuleExecutionData[] memory moduleExecutionDataBtc = new DataTypes.ModuleExecutionData[](1);
        moduleExecutionDataBtc[0] = DataTypes.ModuleExecutionData({
            executionType: DataTypes.CallType.DELEGATECALL,
            module: address(emodeModule),
            data: abi.encodeWithSelector(emodeModule.execute.selector, paramsBtc)
        });

        vm.startPrank(admin);
        superloop.operate(moduleExecutionData);
        superloopBtc.operate(moduleExecutionDataBtc);
        ...
        superloopBtc.setRegisteredModule(REPAY_MODULE, true);
        superloopBtc.setRegisteredModule(WITHDRAW_MODULE, true);
        superloopBtc.setRegisteredModule(DEPOSIT_MODULE, true);
        superloopBtc.setRegisteredModule(BORROW_MODULE, true);
        superloopBtc.setRegisteredModule(DEX_MODULE, true);
        vm.stopPrank();

        vm.label(oldVault, "oldVault");
        vm.label(oldVaultBtc, "oldVaultBtc");
        vm.label(address(superloop), "newVault");
        vm.label(address(superloopBtc), "newVaultBtc");
        ...
        // TODO: do deposit and withdraw for both
    }
    ...
    //     address[] memory users = ALL_HOLDERS_XTZ;
    ...
    //     uint256 batches = 4;
    //     migrationHelper.migrate(oldVault, address(superloop), users, ST_XTZ, XTZ, batches, maxSharesDelta, 5);
    ...
    function test_migrationBtc() public {
        // deploy migration helper
        MigrationHelper migrationHelper = new MigrationHelper(
            ...
            BORROW_MODULE,
            DEX_MODULE,
            environment.stablecoin
        );
        vm.label(address(migrationHelper), "migrationHelper");

        // set migration helper contract as priveledged address in old vault
        address oldVaultAdmin = ISuperloop(oldVaultBtc).vaultAdmin();
        vm.prank(oldVaultAdmin);
        ISuperloop(oldVaultBtc).setPrivilegedAddress(address(migrationHelper), true);

        // set migration helper contract as deposit manager and vault operator in new vault
        vm.startPrank(admin);
        superloopBtc.setDepositManagerModule(address(migrationHelper));
        superloopBtc.setVaultOperator(address(migrationHelper));

        // set migration helper as vault in new accountant module
        accountantBtc.setVault(address(migrationHelper));
        accountantBtc.transferOwnership(address(migrationHelper));
        vm.stopPrank();

        address[] memory users = ALL_HOLDERS_BTC;

        // old balances
        uint256 oldVaultBTCBalance = IERC20(WBTC).balanceOf(oldVaultBtc);
        uint256 oldVaultLBTCBalance = IERC20(LBTC).balanceOf(oldVaultBtc);
        (uint256 oldVaultLendBalance,,,,,,,,) = poolDataProvider.getUserReserveData(LBTC, oldVaultBtc);
        (,, uint256 oldVaultBorrowBalance,,,,,,) = poolDataProvider.getUserReserveData(WBTC, oldVaultBtc);

        console.log("OLD VAULT STATE INITIAL");
        console.log("oldVaultBTCBalance", oldVaultBTCBalance);
        console.log("oldVaultLBTCBalance", oldVaultLBTCBalance);
        console.log("oldVaultLendBalance", oldVaultLendBalance);
        console.log("oldVaultBorrowBalance", oldVaultBorrowBalance);
        console.log("old vault total supply", ISuperloop(oldVaultBtc).totalSupply());
        console.log("old vault total assets", ISuperloop(oldVaultBtc).totalAssets());

        assertEq(Ownable(address(accountantBtc)).owner(), address(migrationHelper));
        assertEq(accountantBtc.vault(), address(migrationHelper));
        ...
        uint256 startGas = gasleft();
        uint256 batches = 4;
        migrationHelper.migrate(oldVaultBtc, address(superloopBtc), users, LBTC, WBTC, batches, maxSharesDelta, 5);
        ...
        uint256 gasUsed = startGas - endGas;
        console.log("gas used", gasUsed);
        uint256 newVaultBTCBalance = IERC20(WBTC).balanceOf(address(superloopBtc));
        uint256 newVaultLBTCBalance = IERC20(LBTC).balanceOf(address(superloopBtc));
        (uint256 newVaultLendBalance,,,,,,,,) = poolDataProvider.getUserReserveData(LBTC, address(superloopBtc));
        (,, uint256 newVaultBorrowBalance,,,,,,) = poolDataProvider.getUserReserveData(WBTC, address(superloopBtc));
        uint256 newVaultLastRealizedFeeExchangeRate =
            IAccountantModule(ISuperloop(address(superloopBtc)).accountant()).lastRealizedFeeExchangeRate();
        uint256 oldVaultLastRealizedFeeExchangeRate =
            IAccountantModule(ISuperloopLegacy(oldVaultBtc).accountantModule()).lastRealizedFeeExchangeRate();

        console.log("NEW VAULT STATE AFTER MIGRATION");
        console.log("newVaultXTZBalance", newVaultBTCBalance);
        console.log("newVaultLBTCBalance", newVaultLBTCBalance);
        console.log("newVaultLendBalance", newVaultLendBalance);
        console.log("newVaultBorrowBalance", newVaultBorrowBalance);
        console.log("new vault total supply", ISuperloop(address(superloopBtc)).totalSupply());
        console.log("new vault total assets", ISuperloop(address(superloopBtc)).totalAssets());
        console.log(
            "new vault last realized fee exchange rate",
            IAccountantModule(ISuperloop(address(superloop)).accountant()).lastRealizedFeeExchangeRate()
        );
        ...
        for (uint256 i = 0; i < users.length; i++) {
            assertEq(ISuperloop(address(superloopBtc)).balanceOf(users[i]), ISuperloop(oldVaultBtc).balanceOf(users[i]));
        }

        assertEq(newVaultLastRealizedFeeExchangeRate, oldVaultLastRealizedFeeExchangeRate);
        assertEq(Ownable(address(accountantBtc)).owner(), address(admin));
        assertEq(accountantBtc.vault(), address(superloopBtc));

        console.log("gas used", gasUsed);

        oldVaultBTCBalance = IERC20(WBTC).balanceOf(oldVaultBtc);
        oldVaultLBTCBalance = IERC20(LBTC).balanceOf(oldVaultBtc);
        (oldVaultLendBalance,,,,,,,,) = poolDataProvider.getUserReserveData(LBTC, oldVaultBtc);
        (,, oldVaultBorrowBalance,,,,,,) = poolDataProvider.getUserReserveData(WBTC, oldVaultBtc);
