...
contract Deploy is Script {
    ...
    function setUp() public {
        asset = XTZ; // WBTC
        name = "Superloop XTZ"; // "Superloop WBTC"
        symbol = "sloopXTZ"; // "sloopWBTC"
        supplyCap = 760_000 * XTZ_SCALE; // 12.5 * BTC_SCALE
        minimumDepositAmount = 100; // in token terms
        instantWithdrawFee = 10; // 0.1% in BPS
        cashReserve = 100; // 1% in BPS
        performanceFee = 1500; // 10% in bps
        lendAsset = ST_XTZ; // LBTC
        borrowAsset = XTZ; // WBTC
        seedAmount = 0;
        emodeCategory = 3; // 2;
    ...
    function run() public {
        ...
        // call emode module and setup emode
        _setupEmode();
    ...
    function _setupEmode() internal {
        DataTypes.AaveV3EmodeParams memory params = DataTypes.AaveV3EmodeParams({emodeCategory: emodeCategory});
