...
abstract contract SuperloopAccountantAaveV3Storage is
    SuperloopAccountantAaveV3Base
{
    uint16 public constant MAX_PERFORMANCE_FEE = 1000; // 10%

    function _setPoolAddressesProvider(
        address poolAddressesProvider_
    ) internal {
        Storages.SuperloopAccountantAaveV3State
            storage $ = _getSuperloopAccountantAaveV3Storage();
